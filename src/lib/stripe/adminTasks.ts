...
export const manageSubscriptionStatusChange = async (
    ...
    try {
      ...
      if (!customerData) {
        throw new Error('Cannot find the customer in the database');
      }
      ...
      const subscriptionData: Subscription = {
        ...
        priceId: subscription.items.data[0].price.id,
        //@ts-ignore
        quantity: subscription.quantity ?? 1, // Defaulting to 1 if undefined
        cancelAtPeriodEnd: subscription.cancel_at_period_end,
        created: toDateTime(subscription.created).toISOString(),
        currentPeriodStart: toDateTime(
          subscription.current_period_start
      ...
      console.log(
        `Subscription [${subscription.id}] for user [${uuid}] has been inserted/updated`
      );
